/**
 * Gateway Payment Routes — Auth and routing smoke tests
 */
...
describe('Gateway Payment Routes', () => {
  describe('GET /api/v1/payments/transactions/me', () => {
    ...
    it('accepts authenticated requests and proxies to payment service', async () => {
      const token = makeToken();
      const res = await request(app)
        .get('/api/v1/payments/transactions/me')
  ...
  describe('GET /api/v1/payments/admin/orders', () => {
    it('requires authentication', async () => {
      const res = await request(app).get('/api/v1/payments/admin/orders');

      expect(res.status).toBe(401);
    });

    it('proxies admin route when authenticated', async () => {
